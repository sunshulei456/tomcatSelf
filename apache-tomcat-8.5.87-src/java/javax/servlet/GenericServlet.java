...
/**
 * <p>
 *      一个通用的，但是和协议无关的 servlet, 是用来简化 servlet 的开发，也可以理解为一个简单案例。
 *    你想要自己实现一个自己定义的协议的 servlet, 就可以通过重写该类的方法来实现。
 *    当然如果是常见协议，最好是扩展已知类型的servlet,比如你想自己写一个针对 http 协议的 servlet, 最好是扩展 httpServlet
 *    <br/>
 *      使编写servlet变得更容易。它提供了生命周期方法init和destroy以及Servlet Config接口中方法的简单版本。GenericServlet还实现了在Servlet上下文接口中声明的log方法。
 *      要编写通用servlet，您只需要覆盖抽象服务方法。
 *    <br/>
 *      实现了ServletConfig接口，所以可以直接调用getInitParameter()、getServletContext()等ServletConfig的方法。
 * </p>
...
public abstract class GenericServlet implements Servlet, ServletConfig,
    ...
    /**
     * <p>
     *     增加一个空参init方法,供开发人员初始化，为了防止开发人员重写 原生init方法
     * </p>
    ...
    /**
     * <P>
     *     仅仅在此声明。交由子类去实现具体的应用, 各个协议根据自己的情况去处理请求、相应，具有很强的扩展性
