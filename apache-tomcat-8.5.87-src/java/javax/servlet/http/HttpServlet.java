 ...
 * <p>
 *     设计成抽象类的原因是不允许他被实例化
 *      因为它处理的是http 的请求, 至于处理什么样的请求 ,如何处理请求需要由我们自己编写的Servlet 来设计
 * </p>
 * <p>
 *     http请求： doxxx[get,head,post,put,delete,options, trace]
 * </p>
...
public abstract class HttpServlet extends GenericServlet {
    ...
    /**
     * get_last_modified: 【modified：英   /ˈmɒdɪfaɪd/ 调整，修改】
     * <p>
     *  用于确定上次修改请求的时间。需重写该方法并发牛一个时间戳。在执行 doGet 等方法之前，
     *  先调用该方法来检查是否需要重新生成相应。
     *  如果时间戳与客户端的`If-Modified-Since`请求头中提供的时间戳相同或更早，那么容器将返回一个`304 Not Modified`响应，告诉客户端使用缓存的响应。
     *  如果时间戳比客户端提供的时间戳更晚，那么容器将执行`doGet()`、`doPost()`等方法，生成新的响应并将其发送回客户端。
     *
     * </p>
     *
    ...
    /**
     * <p>
     *     处理 http head 请求。 head 请求和 get 请求类似，但是服务器不会返回任何响应保温，只是返回响应头信息。
     *     该方法被子类覆盖以提供自定义的响应头信息，如果子类没有覆盖该方法，则默认调用 doGet 方法， 只是不会返回响应正文
     * </p>
     * <p>
     ...
     * protocol 英  /ˈprəʊtəkɒl/  协议
     * <p>
     *  用于处理不允许的HTTP方法的方法， 目前主要处理的额 http 版本是 http 1.1
     *
     * </p>
     * @param req
     * @param resp
    ...
    /**
     * <p>
     *     可以用 options 请求去嗅探某个请求在对应的服务器中都支持哪种请求方法。
     *     <br/>
     *     案例一： 跨域共享标准规范要求，对那些可能对服务器数据产生副作用的 HTTP 请求方法（特别是 GET 以外的 HTTP 请求，
     ...
     *     2. 对 options 请求进行缓存，服务器端设置 Access-Control-Max-Age 字段，那么当第一次请求该 URL 时会发出 OPTIONS 请求，
     *     浏览器会根据返回的 Access-Control-Max-Age 字段缓存该请求的 OPTIONS 预检请求的响应结果（具体缓存时间还取决于浏览器的支持的默认最大值，取两者最小值，一般为 10 分钟）。
     *     在缓存有效期内，该资源的请求（URL 和 header 字段都相同的情况下）不会再触发预检。
     *     （chrome 打开控制台可以看到，当服务器响应 Access-Control-Max-Age 时只有第一次请求会有预检，后面不会了。
     ...
     * trace 追踪;追溯
     * <p>
     *     是HTTP（超文本传输）协议定义的一种协议调试方法，该方法使得服务器原样返回任何客户端请求的内容（可能会附加路由中间的代理服务器的信息），
     *     由于该方法原样返回客户端提交的任意数据，因此，可用来进行跨站脚本（XSS）攻击，这种攻击方式又称为跨站跟踪攻击（XST）。
     * </p>
